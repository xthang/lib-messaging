...
if [ "$1" = 'desktop' ];
then
    echo_then_run cargo build -p lib-messaging-jni --release
    copy_built_library target/release messaging_jni "${DESKTOP_LIB_DIR}/"
elif [ "$1" = 'android' ];
then
    ...
    echo_then_run cargo build -p lib-messaging-jni --release -Z unstable-options --target aarch64-linux-android --out-dir "${ANDROID_LIB_DIR}/arm64-v8a"
    echo_then_run cargo build -p lib-messaging-jni --release -Z unstable-options --target armv7-linux-androideabi --out-dir "${ANDROID_LIB_DIR}/armeabi-v7a"
    echo_then_run cargo build -p lib-messaging-jni --release -Z unstable-options --target x86_64-linux-android --out-dir "${ANDROID_LIB_DIR}/x86_64"
    echo_then_run cargo build -p lib-messaging-jni --release -Z unstable-options --target i686-linux-android --out-dir "${ANDROID_LIB_DIR}/x86"
else
