...
ignore_this_warning = re.compile(
    ...
    r"WARN: Skip lib-messaging-bridge::.+ - \(not `(?:pub|no_mangle)`\)\.|"
...
java_decl = re.compile(r'([a-zA-Z]+) Java_com_cystack_ready_libmessaging_internal_Native_([A-Z][a-zA-Z0-9]+)_1([A-Za-z0-9]+)\(JNIEnv .?env, JClass class_(, .*)?\);')
...
native_java = os.path.join(our_abs_dir, '../../../../java/shared/java/com/cystack/ready/libmessaging/internal/Native.java')
