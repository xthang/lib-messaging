...
//     gcm: Option<messaging_crypto::Aes256GcmEncryption>,
// }

// impl Aes256GcmEncryption {
//     pub fn new(key: &[u8], nonce: &[u8], associated_data: &[u8]) -> Result<Self> {
//         let gcm = messaging_crypto::Aes256GcmEncryption::new(key, nonce, associated_data)?;
...
//     gcm: Option<messaging_crypto::Aes256GcmDecryption>,
// }

// impl Aes256GcmDecryption {
//     pub fn new(key: &[u8], nonce: &[u8], associated_data: &[u8]) -> Result<Self> {
//         let gcm = messaging_crypto::Aes256GcmDecryption::new(key, nonce, associated_data)?;
//         Ok(Self { gcm: Some(gcm) })
//     }
...
#[bridge_fn(ffi = "aes256_cbc_encrypt")]
fn Aes256Cbc_Encrypt(
    ptext: &[u8],
    key: &[u8],
    iv: &[u8],
) -> Result<Vec<u8>> {
    let encrypted = aes_256_cbc_encrypt(ptext, key, iv).map_err(|e| {
        log::error!("[aes256_cbc_encrypt] ERROR");
        Error::EncryptionError(e.to_string())
    })?;
    Ok(encrypted)
}

#[bridge_fn(ffi = "aes256_cbc_decrypt")]
fn Aes256Cbc_Decrypt(
    ptext: &[u8],
    key: &[u8],
    iv: &[u8],
) -> Result<Vec<u8>> {
    let decrypted = aes_256_cbc_decrypt(ptext, key, iv).map_err(|e| {
        log::error!("[aes_256_cbc_decrypt] ERROR");
