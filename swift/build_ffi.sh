...
echo_then_run cargo build -p lib-messaging-ffi ${RELEASE_BUILD:+--release} ${VERBOSE:+--verbose} ${CARGO_BUILD_TARGET:+--target $CARGO_BUILD_TARGET} ${BUILD_STD:+-Zbuild-std}

FFI_HEADER_PATH=swift/Sources/MessagingFfi/messaging_ffi.h
...
if [[ -n "${SHOULD_CBINDGEN}" ]]; then
  ...
  if [[ -n "${CBINDGEN_VERIFY}" ]]; then
    ...
    if ! diff -u "${FFI_HEADER_PATH}"  <(cbindgen -q ${RELEASE_BUILD:+--profile release} rust/bridge/ffi); then
      echo
      echo 'error: messaging_ffi.h not up to date; run' "$0" '--generate-ffi' >&2
      exit 1
    fi
  else
